...
!     special case: plane stress and plane strain elements
!
...
c      enddo
      do j=1,3
         xs(1,j)=xl(1,1)*(shpe(j,1)+shpe(j,5)+shpe(j,17))
     &          +xl(1,2)*(shpe(j,2)+shpe(j,6)+shpe(j,18))
     &          +xl(1,3)*(shpe(j,3)+shpe(j,7)+shpe(j,19))
     &          +xl(1,4)*(shpe(j,4)+shpe(j,8)+shpe(j,20))
