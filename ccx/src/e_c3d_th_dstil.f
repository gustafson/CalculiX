...
!
      subroutine e_c3d_th_dstil(co,nk,kon,lakonl,s,sm,
...
!
         ...
         do i1=1,nope
            ...
            shpj(3,i1)=shptil(3,i1)*xsjj
            shpj(4,i1)=shptil(4,i1)*xsj
         enddo
!
...
!
                   ff(ipointer)=ff(ipointer)+shptil2(4,k)*xload(1,id)
     &                  *areaj
                elseif(sideload(id)(1:1).eq.'F') then
                   ff(ipointer)=ff(ipointer)+shptil2(4,k)*areaj*
     &                  (heatfac-xload(1,id)*(temp-sinktemp))
                elseif(sideload(id)(1:1).eq.'R') then
                   ff(ipointer)=ff(ipointer)-shptil2(4,k)*physcon(2)*
...
!
     ...
     &                  shptil2(4,ii)*shptil2(4,jj)*xload(1,id)*areaj
                   elseif(sideload(id)(1:1).eq.'R') then
                      s(ipointeri,ipointerj)=s(ipointeri,ipointerj)+
     &                     shptil2(4,ii)*shptil2(4,jj)*physcon(2)
     &                     *xload(1,id)*areaj*4.d0*(temp-physcon(1))**3
                   endif
                enddo
             enddo
!
