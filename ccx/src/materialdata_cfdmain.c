...
static ITG *nef1,*nshcon1,*ielmatf1,*ntmat1_,*mi1,*ithermal1,*nrhcon1,
    *ncocon1,*ielfa1,*num_cpus1,*nface1;

static double *vel1,*shcon1,*cvel1,*physcon1,*umel1,*rhcon1,*vfa1,*cocon1,
    *cvfa1,*umfa1,*hcfa1;

void materialdata_cfdmain(ITG *nef,double *vel,double *shcon,ITG *nshcon,
			  ...
			  double *hcfa,double *rhcon,ITG *nrhcon,ITG *num_cpus){
    ...
    umel1=umel;rhcon1=rhcon;nrhcon1=nrhcon;num_cpus1=num_cpus;
	...
	ithread[i]=i;
	pthread_create(&tid[i], NULL, (void *)materialdata_cfd1mt, (void *)&ithread[i]);
    }
    ...
    umfa1=umfa;ielfa1=ielfa;hcfa1=hcfa;rhcon1=rhcon;nrhcon1=nrhcon;
    ...
    for(i=0; i<*num_cpus; i++)  {
	ithread[i]=i;
	pthread_create(&tid[i], NULL, (void *)materialdata_cfd2mt, (void *)&ithread[i]);
    }
    for(i=0; i<*num_cpus; i++)  pthread_join(tid[i], NULL);
...
void *materialdata_cfd1mt(ITG *i){
    ...
    FORTRAN(materialdata_cfd1,(nef1,vel1,shcon1,nshcon1,ielmatf1,
			       ntmat1_,mi1,cvel1,physcon1,ithermal1,
			       umel1,rhcon1,nrhcon1,&nefa,&nefb));

    return NULL;
}

/* subroutine for multithreading of materialdata_cfd2 */
...
void *materialdata_cfd2mt(ITG *i){
    ...
    FORTRAN(materialdata_cfd2,(shcon1,nshcon1,ielmatf1,ntmat1_,
			       mi1,vfa1,cocon1,ncocon1,physcon1,
			       cvfa1,ithermal1,umfa1,ielfa1,hcfa1,
			       rhcon1,nrhcon1,&nfacea,&nfaceb));
