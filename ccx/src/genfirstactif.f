...
!
c>
c> \brief subroutine generating the first actice set 
...
c> @param [in] itietri          (1,i) first triangle in field koncont for contact contraint i (2,i) last one
...
c> @param [in] iit		iteration number of Newton-Raphson iteration
...
c> @param [in] ne0              number of elements without contact elements
...
c> @param [out] islavact	(i) indicates, if slave node i is active (=-3 no-slave-node, =-2 no-LM-node, =-1 no-gap-node, =0 inactive node, =1 sticky node, =2 slipping/active node) 
...
c     write(*,*) 'genfirstactiv tie',tieset(3,i)
         kneigh=1
         slavset=tieset(2,i)
...
c     write(*,*) 'genfirstactiv: tie',i,'n',nstart,n
