...
!     
!     viscoplastic material without plasticity (zero yield surface)
!     
          iperturb(1)=3
          nelcon(1,nmat)=-54
          do i=2,n
...
!     
!     note: if the *CREEP card is not preceded by a *PLASTIC card, a zero
...
!     
     ...
     &             "*CREEP%",ier)
          ...
          elcon(16,ntmat,nmat)=temperature
        enddo
!     
!     interpolating the creep data at the elastic temperature
!     data points
!     
!     before interpolation: data are stored in positions 13-16:
!     A,n,m,temperature
!     after interpolation: data are stored in positions 10-12:
!     A,n,m
!     
        write(*,*) '*INFO: interpolating the creep data at the'
        write(*,*) '       elastic temperature data points;'
...
!     
        do i=1,nelcon(2,nmat)
          t1l=elcon(0,i,nmat)
          call ident2(elcon(16,1,nmat),t1l,ntmat,ncmat_+1,id)
          if(ntmat.eq.0) then
            continue
          elseif((ntmat.eq.1).or.(id.eq.0)) then
            elcon(10,i,nmat)=elcon(13,1,nmat)
            elcon(11,i,nmat)=elcon(14,1,nmat)
            elcon(12,i,nmat)=elcon(15,1,nmat)
          elseif(id.eq.ntmat) then
            elcon(10,i,nmat)=elcon(13,id,nmat)
            elcon(11,i,nmat)=elcon(14,id,nmat)
            elcon(12,i,nmat)=elcon(15,id,nmat)
          else
            do k=10,12
              elcon(k,i,nmat)=elcon(k+3,id,nmat)+
     &             (elcon(k+3,id+1,nmat)-elcon(k+3,id,nmat))*
     &             (t1l-elcon(16,id,nmat))/
     &             (elcon(16,id+1,nmat)-elcon(16,id,nmat))
            enddo
          endif
          write(*,*) t1l,(elcon(k,i,nmat),k=10,12)
        enddo
        write(*,*)
!     
      endif
