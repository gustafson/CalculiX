...
!     
        if(itie.eq.0) then
          write(*,*)'*ERROR reading *MODEL CHANGE: contact pair '
          write(*,*) '      does not exist. Notice that slave'
          write(*,*) '      surface must come first.'
          call inputerror(inpc,ipoinpc,iline,"*MODEL CHANGE%",ier)
          return
        endif
!     
        if(add) then
