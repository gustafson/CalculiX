...
!
     ...
     &  fmpc(*),shcon,sph,c1,vel(1:3,20),veoldl(3,9)
!
...
c            if((nmethod.eq.2).or.(nmethod.eq.3).or.(iperturb.eq.0))
            if((nmethod.eq.2).or.(nmethod.eq.3).or.
     &            ((iperturb.eq.0).and.(nmethod.eq.1)))
     &         then
               incrementalmpc=0
            else
               incrementalmpc=1
...
! 
      if((ithermal.le.1).or.(ithermal.ge.3)) then
!
      do i=1,ne
!
