...
!
      integer kon(*),konl(20),inum(*),iperm(20),
...
!
      integer nk,ne,mattyp,ithermal,iprestr,i,j,k,m1,m2,jj,
     ...
     &  nset,istartset(*),iendset(*),ialset(*),nprint,ntrans
!
...
!
     ...
     &  alcon(0:6,ntmat_,*),vini(0:3,*),qfx(3,mint_,*),qfn(3,*),
     &  alzero(*),orab(7,*),elas(21),rho,f(*),fn(0:3,*),
     &  skl(3,3),beta(6),q(0:3,20),vkl(0:3,3),cam(2),
     &  t0(*),t1(*),prestr(6,mint_,*),eme(6,mint_,*),een(6,*),ckl(3,3),
     &  vold(0:3,*),b(*),xboun(*),coefmpc(*),eloc(9),veold(0:3,*),
     ...
     &  fmpc(*),shcon,sph,c1,vel(1:3,20)
!
      real*8 e,un,al,um,am1,xi,et,ze,tt,exx,eyy,ezz,exy,exz,eyz,
     &  xsj,qa(2),vj,t0l,t1l,bet,gam,dtime,forcempc,scal1,scal2,bnac,
...
!
      if((iperturb.ge.2).or.((iperturb.le.0).and.(iout.lt.0))) then
         if(iout.lt.1) then
            calcul_fn=.true.
            calcul_f=.true.
            calcul_qa=.true.
...
c         write(*,*) 'boun ',nodeboun(i),ndirboun(i),fixed_disp
      enddo
c      write(*,*) 'start olddisp'
c      do i=1,nk
c         write(*,*) i,(vold(j,i),j=0,3)
c      enddo
c      write(*,*) 'end olddisp'
c      write(*,*) 'start disp'
c      do i=1,nk
c         write(*,*) i,(v(j,i),j=0,3)
c      enddo
c      write(*,*) 'end disp'
!
...
!
      ...
      if(filab(7)(1:4).eq.'ENER') then
         ...
         do i=1,nprint
            if((prlab(i)(1:4).eq.'ENER').or.(prlab(i)(1:4).eq.'ELSE')) 
     &        then
               iener=1
               exit
            endif
...
!
c         if((iperturb.ge.2).or.((iperturb.eq.0).and.(iout.lt.1))) then
         if((iperturb.ge.2).or.((iperturb.le.0).and.(iout.lt.1))) then
            do m1=1,nope
               do m2=1,3
...
!
     ...
     &              fn,ncmat_,ntmat_,nope,lakonl,t0l,t1l,kode,elconloc,
     ...
     &              elconloc,plicon,nplicon,npmat_,vel)
            endif
         endif
!
         do jj=1,mint3d
...
!
c            if((iperturb.eq.1).or.((iperturb.eq.0).and.(iout.lt.0)))then
            if((iperturb.eq.1).or.(iperturb.eq.-1))then
               do m2=1,3
                  do m3=1,3
                     vokl(m2,m3)=0.d0
                  enddo
...
!
c            if((iperturb.ne.0).or.(iout.ge.0)) then
...
!
            ...
            enddo
!
c            if((iperturb.eq.0).and.(iout.lt.0)) then
            if(iperturb.eq.-1) then
!
...
c      write(*,*) 'nodal force'
c      do i=1,nk
c         write(*,*) i,fn(1,i),fn(2,i),fn(3,i)
c      enddo
!
      endif
!
...
!
      if(calcul_fn)then
        do i=1,nmpc
            ...
            node=nodempc(1,ist)
            ndir=nodempc(2,ist)
c            write(*,*) 'v 3,129 2,262',v(3,129),v(2,262)
c            write(*,*) 'forcempc',node,ndir,fn(ndir,node),coefmpc(ist)
...
!     step)
!
c      if((filab(1)(1:4).eq.'U   ').or.
c     &     ((filab(2)(1:4).eq.'NT  ').and.(ithermal.gt.1))) then
c         if(filab(1)(5:5).eq.'I') then
            nfield=4
            cflag=filab(1)(5:5)
...
!
      call uout(v)
!
      if((filab(2)(1:4).eq.'NT  ').and.(ithermal.le.1)) then
...
c      if((filab(5)(1:4).eq.'RF  ').or.(filab(10)(1:4).eq.'RFL '))then
c         if(filab(1)(5:5).eq.'I') then
c            nfield=4
...
!
     ...
     &        nelemload,nload,nodeboun,nboun)
      endif
!
...
!
     ...
     &    (filab(6)(1:4).ne.'PE  ').and.(filab(7)(1:4).ne.'ENER').and.
