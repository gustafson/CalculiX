...
!
      subroutine resultstherm_dstil(co,kon,ipkon,lakon,v,
...
!
     ...
     &  islavelinv(*),islavsurf(*),node1,node2,
     &  ii,j1,j2     
!
...
!
     ...
     &               -c1*(qflux(1)*(shptil(1,m1)+shptil(1,iperm(m1)))
     &                  +qflux(2)*(shptil(2,m1)+shptil(2,iperm(m1)))
     &                  +qflux(3)*(shptil(3,m1)+shptil(3,iperm(m1))))
                  enddo
               else
                  do m1=1,nope
                     do m3=1,3
                        fn(0,konl(m1))=fn(0,konl(m1))-
     &                       c1*qflux(m3)*shptil(m3,m1)
                     enddo
                  enddo
               endif
            endif
