...
!
      subroutine cavityext(kontet,ifatet,ifreetet,bc,ifac,itetfa,
...
!
     ...
     &  ifcav(4*netet_),kflag,ng,necav,nkcav,nfcav,iface,iel,k,ichange,
     &  id,idum,ielnew,j,ifanew,ielement,l,inode,ifreele,inewel(netet_),
     &  iexternedg(*),iedtet(6,*),iedge,iexternnode(*),node1,node2,
     &  n1,n2,n3,indexe,indexeold,index,indexold,jface
!
      real*8 bc(4,*),planfa(4,*),cotet(3,*),cotetorig(3),cg(3,*),
     &  dd,x,y,z,conewnodes(3,*),volume,height,hh,hei(4),surf,
     &  cotetcpy(3)
!
      intent(in) ibase,node,netet_,conewnodes,nnewnodes,height
!
      intent(inout) ibasenewnodes,kontet,ifreeed,iedg,ipoed,ifreele,
     &  ieled,ipoeled,bc,ifatet,planfa,iedtet,nktet,cotet,
     &  ifreeln,ieln,ipoeln,ifreefa,ifac,ipofa,itetfa,ifreetet,
     &  iexternfa,iexternedg,cg
!
...
c      call isortii(ikcav,idum,nkcav,kflag)
!
      nfcav=0
!
...
!
     ...
     &         (ielement.ne.ibase)) then
               write(*,*) '*INFO in cavityext: element is removed'
               write(*,*) '      from cavity'
            else
               cycle
            endif
...
!
         call removetet2(kontet,ifatet,ifreetet,ifac,itetfa,
...
!
         inewel(i)=ifreetet
         call generatetet2(kontet,ifatet,ifreetet,bc,ifac,itetfa,
...
!
     ...
     &      (hei(4).lt.hh/100.d0)) then
            irevert=1
            nktet=nktet-1
            write(*,*) '*INFO in cavityext: bad element'
            write(*,*) '      node is not inserted'
!
...
!
            do j=1,necav
               ...
               enddo
               inewel(j)=ifreetet
               call generatetet2(kontet,ifatet,ifreetet,bc,ifac,
...
!
                  if(isol.eq.0) then
                     write(*,*) '*ERROR in cavity: node belongs to'
                     write(*,*) '       zero elements'
                     call exit(201)
                  endif
               else
...
!
                  if(iexternfa(iface).eq.0) then
                     write(*,*) '*ERROR in cavityext: external'
                     ...
                     if(iedg(2,indexe).eq.n2) then
                        if(n1.eq.node) then
                           if((n2.eq.node1).or.(n2.eq.node2)) then
                              iexternedg(indexe)=iexternedg(iedge)
                           else
...
!
         ...
         if(index.eq.iface) then
            ...
            do
               ...
               index=ifac(4,index)
               if(index.eq.0) then
                  write(*,*) '*ERROR in cavityext: face to be'
