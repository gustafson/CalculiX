...
!
      subroutine cavity(kontet,ifatet,ifreetet,bc,ifac,itetfa,
     ...
     &     nnewnodes,iexternedg,iedtet,cg,height)
!
      implicit none
!
!     ig(*): face queue
...
!
     ...
     &  dd,x,y,z,conewnodes(3,*),hei(4),volume,surf
!
      intent(in) cotet,ibase,node,iexternfa,height,
     &     netet_,conewnodes,nnewnodes,iexternedg
!
      intent(inout) ibasenewnodes,kontet,ifreeed,iedg,ipoed,ifreele,
     &  ieled,ipoeled,bc,ifatet,planfa,iedtet,nktet,
...
!
c         write(*,*) 'cavity4 ',i,ifcav(i),volume
         do j=1,4
            iface=abs(ifatet(j,inewel(i)))
c            write(*,*) 'cavity4a ',ifac(1,iface),ifac(2,iface),
c     &            ifac(3,iface)
            call calcsurf(ifac(1,iface),ifac(2,iface),ifac(3,iface),
     &                    cotet,surf)
c         write(*,*) 'cavity5 ',i
c         write(*,*) 'cavity4b ',j,i,ifcav(i),surf
            hei(j)=volume/surf
         enddo
c         write(*,*) 'cavity5 ',i
!
...
!
     ...
     &      (hei(4).lt.hh/100.d0)) then
            irevert=1
            nktet=nktet-1
            write(*,*) '*INFO in cavity: bad element'
