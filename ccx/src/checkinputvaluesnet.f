...
!               if IDs: the IDs stored in prop_store array are copied back in prop array
!
!    2016.05.19 Y. Muller
!
...
!
     ...
     &           prop(index+4).gt.0.01d0)then
               write(*,*) '*ERROR: readingin propertynet.f:'
...
!
     ...
     &          (lakon(nelem)(2:5).ne.'ORFL'))then
            if(prop(index+2).lt.0.d0) then
               write(*,*) '*ERROR reading *FLUID SECTION: diameter '
     ...
     &                     not positive'
            ...
            if((lakon(nelem)(2:5).ne.'ORRG').and.
     ...
     &            the orifice must be zero or the chamfer angle'
            ...
            if(prop(index+4)/prop(index+2).lt.0.d0) then
               write(*,*) '*ERROR reading *FLUID SECTION: '
               write(*,*) ' r/d of an orifice must not be negative'
               call exit(201)
            endif
...
!
         elseif(lakon(nelem)(2:3).eq.'RE') then
            if(((prop(index+1).le.0)
     &           .or.(prop(index+2).le.0)
     &           .or.(prop(index+3).le.0))
...
!
     ...
     &                        from 0'
                  call exit(201)
!
               elseif((prop(index+4).le.0)
     &                 .or.(prop(index+5).le.0)
     &                 .or.(prop(index+6).le.0))then
                  write(*,*) '*ERROR reading *FLUID SECTION:'
                  write(*,*) 'trying to define a branch '
...
!         inner and outer radius less or equal to 0
            if((prop(index+1).le.0) .or.
     &           (prop(index+2).le.0)) then
               write(*,*)'*ERROR reading *FLUID SECTION:'
               write(*,*)'trying to define a VORTEX'
               write(*,*)'R1 and R2 must be positive'
               call exit(201)
!
            elseif(prop(index+3).le.0) then
               write(*,*)'*ERROR reading *FLUID SECTION:'
               write(*,*)'trying to define a VORTEX'
...
!
     ...
     &          (lakon(nelem)(2:4).eq.'RTA')) then
            ...
            elseif(prop(index+3).ne.0) then
               if(prop(index+2).ne.0) then
                  write(*,*)'*ERROR reading *FLUID SECTION:'
                  write(*,*)'trying to define an element'
                  write(*,*)'TYPE= ABSOLUTE TO RELATIVE or'
                  write(*,*)'TYPE= RELATIVE TO ABSOLUTE'
                  write(*,*)'reference element has been provided'
                  write(*,*)'but tengential velocity
     &                       is already defined'
                  call exit(201)
               endif
            endif
