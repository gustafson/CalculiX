...
!     
     ...
     &     xxmid,xxmax,dummy,a(3,3),b(3,3),c(3,3),
...
!
cc            read(10)((f(i,j),j=1,ntr),i=1,ntr),(fenv(i),i=1,ntr)
c            do i=1,ntr
c               do j=1,ntr
c                  read(10) f(i,j)
c               enddo
...
!     
     ...
     &              pmid(3,j)*e3(3,i)+e3(4,i).le.sidemean/80.d0) cycle
               if(pmid(1,i)*e3(1,j)+pmid(2,i)*e3(2,j)+
     &              pmid(3,i)*e3(3,j)+e3(4,j).le.sidemean/80.d0) cycle
c     if(pmid(1,j)*e3(1,i)+pmid(2,j)*e3(2,i)+
c     &            pmid(3,j)*e3(3,i)+e3(4,i).le.1.d-4) cycle
c     if(pmid(1,i)*e3(1,j)+pmid(2,i)*e3(2,j)+
c     &            pmid(3,i)*e3(3,j)+e3(4,j).le.1.d-4) cycle
!
               if(j.gt.ntri) then
                  jmod=mod(j,ntri)
...
c               if(nelemload(2,nloadtr(idi)).ne.
c     &             nelemload(2,nloadtr(idj))) cycle
!
               ndist=ndist+1
...
!     
               do k=1,3
                  p1(k)=co(k,kontri(1,j))+vold(k,kontri(1,j))-pmid(k,i)
               enddo
               dd=dsqrt(p1(1)*p1(1)+p1(2)*p1(2)+p1(3)*p1(3))
               do k=1,3
                  p1(k)=p1(k)/dd
               enddo
...
!     
               do k=1,3
                  p2(k)=co(k,kontri(2,j))+vold(k,kontri(2,j))-pmid(k,i)
               enddo
               dd=dsqrt(p2(1)*p2(1)+p2(2)*p2(2)+p2(3)*p2(3))
               do k=1,3
                  p2(k)=p2(k)/dd
               enddo
...
!     
               do k=1,3
                  p3(k)=co(k,kontri(3,j))+vold(k,kontri(3,j))-pmid(k,i)
               enddo
               dd=dsqrt(p3(1)*p3(1)+p3(2)*p3(2)+p3(3)*p3(3))
               do k=1,3
                  p3(k)=p3(k)/dd
               enddo
...
!
         ...
         enddo
c         write(*,*)
c         write(*,*) 'viewfactor calculation '
c         write(*,*) 'ntr ',ntr
c         write(*,*) 'nonzeros ',nonzero
c         write(*,*) 'percentage nonzero ',real(nonzero)/(ntr*ntr)
c         write(*,*)
!
...
cc            write(10)((f(i,j),j=1,ntr),i=1,ntr),(fenv(i),i=1,ntr)
c            do i=1,ntr
c               do j=1,ntr
c                  write(10) f(i,j)
c               enddo
...
!     
c            if(sideload(i)(5:6).ne.'NU') then
c               erad(i1)=xloadact(1,i)
c            else
c               read(sideload(i)(2:2),'(i1)') jltyp
c               jltyp=jltyp+10
c               call radiate(h(1),h(2),temp,istep,
c     &              iinc,tvar,nelem,l,coords,jltyp,field,nfield,
c     &              sideload(i),node,areaj,vold)
c            endif
!
            e=erad(i1)
            ec=1.d0-e
...
c     if(i1.eq.105) write(*,*) f(i1,j1),f(i1+1,j1),f(i1+2,j1)
...
!
         ...
         enddo
c         write(*,*)
c         write(*,*) 'radiation matrix calculation '
c         write(*,*) 'ntr ',ntr
c         write(*,*) 'nonzeros ',nonzero
c         write(*,*) 'percentage nonzero ',real(nonzero)/(ntr*ntr)
c         write(*,*)
