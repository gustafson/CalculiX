...
void steadystate(double **cop, int *nk, int **konp, int **ipkonp, char **lakonp, int *ne, 
  ...
  int nev,i,j,k, *inum=NULL,*ipobody=NULL,inewton=0,nsectors,im,
    ...
    *ipiv=NULL,info,nev2,ngraph=1,nkg,neg,iflag=1,idummy=1,imax,
    nzse[3],*nnn=*nnnp,mt=mi[1]+1,*ikactmech=NULL,nactmech,id,
...
/*      nev2=2*nev;
  ...
  if(nfour<=0){
	  ...
	  aa=NNEW(double,2*nev); /* modal coefficients of the real loading */
	  bb=NNEW(double,2*nev); /* modal coefficients of the imaginary loading */
      }
	  ...
	  *ttime=time;
	  ...
	  if(iprescribedboundary){
		  ...
		  ikmpc,ilmpc,istep,&iinc,springarea,&reltime,&ne0,
                  xforc,nforc,thicke,xnormastface,shcon,nshcon,
	 ...
	 is not equal to one */
      ...
      if(intpointvar==1){
	  free(fn);
	  free(stnr);free(stni);free(stx);free(eei);
