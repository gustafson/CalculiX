...
!
      subroutine e_c3d_prhs_l(co,nk,konl,lakonl,sm,ff,nelem,nmethod,
     &  rhcon,
     &  nrhcon,ielmat,ntmat_,v,vold,voldaux,nelemface,sideface,nface,
     &  dtl,matname,mi,shcon,nshcon,theta1,physcon,
     &  iexplicit)
!
...
!
            do j1=1,3
               aux(j1)=xsjmod*dtl(konl(jj))*(rhovel(j1)+
     &              theta1*(delrhovel(j1)-dtl(konl(jj))*dpress(j1)))
            enddo
!     
            ff(jj)=ff(jj)+
...
!     
     ...
     &               rhovel(3)*xsj2(3))*xsjmod*dtl(ipointer)
            enddo
         enddo
         id=id-1
      enddo
