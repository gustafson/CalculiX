...
!     
      real*8 fei(4),pi,fmin,fmax,tol,xboun(*),fmpc(*),tmin,alpha(*)
!     
      global=.true.
      cycmpcactive=.true.
!     
...
!     
      ...
      fmax=-1.d0
!     
!     default for minimum time increment in an eventually subsequent
!     explicit dynamic step: determines the selective mass scaling
!     
      tmin=0.d0
!     
!     defaults for alpha (needed to calculate selective mass scaling
!     for an eventually subsequent explicit dynamic step
!     
      alpha(1)=-0.05d0
      alpha(2)=0.5d0
!     
...
!     
      do i=2,n
        ...
        elseif(textpart(i)(1:15).eq.'CYCMPC=INACTIVE') then
          cycmpcactive=.false.
        elseif(textpart(i)(1:6).eq.'ALPHA=') then
     ...
     &           "*FREQUENCY%",ier)
          ...
          if(alpha(1).lt.-1.d0/3.d0) then
            write(*,*) '*WARNING reading *FREQUENCY: alpha is smaller'
            write(*,*) '  than -1/3 and is reset to -1/3'
            alpha(1)=-1.d0/3.d0
          elseif(alpha(1).gt.0.d0) then
            write(*,*) '*WARNING reading *FREQUENCY: alpha is greater'
...
!     
     ...
     &           "*FREQUENCY%",ier)
        ...
        if(textpart(4)(1:1).ne.' ') then
          read(textpart(4)(1:20),'(f20.0)',iostat=istat) tmin
          if(istat.gt.0) then
            call inputerror(inpc,ipoinpc,iline,
     &           "*FREQUENCY%",ier)
            return
