...
!     
               ad(i)=ad(i)+xflux
!
               b(i)=b(i)-(vfa(6,ifa)-vel(i,6))*xflux
!
            else
               if(iel.gt.0) then
!
...
!
                  b(i)=b(i)-(vfa(6,ifa)-vel(iel,6))*xflux
!
               else
!
...
!
!        production term containing the first invariant of the
!        Reynolds stress (= the turbulent kinetic energy)
!
         if(div.gt.0.d0) then
            ad(i)=ad(i)+2.d0*rhovol*div/3.d0
         else
            b(i)=b(i)-2.d0*rhovol*vel(i,6)*div/3.d0
         endif
!
...
!           viscosity
!
            unt=(0.31d0*vel(i,6))/max(0.31d0*vel(i,7),of2(i))
            b(i)=b(i)+rhovol*unt*(
     &           (2.d0*(gradvel(1,1,i)**2+gradvel(2,2,i)**2+
     &                  gradvel(3,3,i)**2)+
     &           (gradvel(1,2,i)+gradvel(2,1,i))**2+
