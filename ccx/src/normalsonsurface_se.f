...
!     
          if((nope.eq.20).or.(nope.eq.8)) then
            do m=1,nopem
              ...
              do k=1,3
                xl2m(k,m)=co(k,nodesurf(m))
              enddo
            enddo
          elseif((nope.eq.10).or.(nope.eq.4)) 
     &           then
            do m=1,nopem
              nodesurf(m)=konl(ifacet(m,jface))
              do k=1,3
                xl2m(k,m)=co(k,nodesurf(m))
              enddo
            enddo
          elseif(nope.eq.15) then
            do m=1,nopem
              nodesurf(m)=konl(ifacew2(m,jface))
              do k=1,3
                xl2m(k,m)=co(k,nodesurf(m))
              enddo
            enddo
            ...
            do m=1,nopem
              nodesurf(m)=konl(ifacew1(m,jface))
              do k=1,3
                xl2m(k,m)=co(k,nodesurf(m))
              enddo
            enddo
...
!     
          if(nopem.eq.8) then
            do m=1,nopem
              node=nodesurf(m)
              if((nodedesiinv(node).eq.1).and.(nnodes.lt.nopedesi))
     &             cycle
              xi=xquad(1,m)
              et=xquad(2,m)
