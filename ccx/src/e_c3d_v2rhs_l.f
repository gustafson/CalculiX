...
!
      subroutine e_c3d_v2rhs_l(co,nk,konl,lakonl,
     &  ff,nelem,nmethod,vold,v,dtl,theta2,iexplicit,mi)
!
...
!     
         if(iexplicit.eq.1) then
            jj1=1
            do jj=1,nope
               term=xsjmod*dtl(konl(jj))*
     &              (shp(4,jj)+dtl(konl(jj))*shpv(jj)/2.d0)
...
!               with stability term 
!
               ff(jj1)=ff(jj1)-xsjmod*dtl(konl(jj))*(dpress(1)*
     &              (shp(4,jj)+(1.d0-theta2)*
     &              dtl(konl(jj))*shpv(jj)/2.d0)+theta2*shp(4,jj)
     &                       *ddpress(1))
               ff(jj1+1)=ff(jj1+1)-xsjmod*dtl(konl(jj))*(dpress(2)*
     &              (shp(4,jj)+(1.d0-theta2)*
     &              dtl(konl(jj))*shpv(jj)/2.d0)+theta2*shp(4,jj)
     &              *ddpress(2))
               ff(jj1+2)=ff(jj1+2)-xsjmod*dtl(konl(jj))*(dpress(3)*
     &              (shp(4,jj)+
     &              (1.d0-theta2)*
     &              dtl(konl(jj))*shpv(jj)/2.d0)+theta2*shp(4,jj)
     &              *ddpress(3))
