...
C
     ...
     &  IEXPY1,IEXPYN,LX,LY,INPX,IART,LSP,IER,NX,NY,L,NA
...
C
  270 IF (IYU .LT. IYO) GO TO 280
      CALL ONEDINT(T(IXU,1),T(IXU,LY),IDX,XA(L),ZA(L),1,INPX,IEXP(1),
     1           IER)
      IF (IER.eq.0) then
         go to 400
      else
...
C
  280 IDY = 0
      ...
      IDY = IDY + 1
      Z1(IDY) = T(1,LL)
      CALL ONEDINT (T(IXU,1),T(IXU,LL),IDX,XA(L),Z2(IDY),1,INPX,
     1    IEXP(1),IER)
      IF (IER.eq.0) then
         go to 290
      else
...
C
  300 CALL ONEDINT (Z1,Z2,IDY,YA(L),ZA(L),1,INPY,IEXP(2),IER)
      IF (IER.eq.0) then
         go to 400
      else
         go to 900
