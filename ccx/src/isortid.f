...
c
c    changed on 01.02.2001: auxiliary array is now real*8
c
...
C   array DY.  The array IX may be sorted in increasing order or
C   decreasing order.  A slightly modified quicksort algorithm is used.
C
C   Description of Parameters
C      IX - integer array of values to be sorted
C      DY - real*8 array to be (optionally) carried along
C      N  - number of values in integer array IX to be sorted
C      KFLAG - control parameter
C            =  2  means sort IX in increasing order and carry DY along.
C            =  1  means sort IX in increasing order (ignoring DY)
C            = -1  means sort IX in decreasing order (ignoring DY)
...
C   901012  Declared all variables; changed X,Y to IX,DY. (M. McClain)
