...
!
      subroutine e_c3d_v1rhs_l(co,nk,konl,lakonl,p1,p2,omx,bodyfx,
     &  nbody,ff,nelem,nmethod,rhcon,nrhcon,ielmat,ntmat_,vold,voldaux,
     &  idist,dtl,matname,mi,
...
c
c        inhomogeneous dtime
c
      integer kflag,idummy
      real*8 proj(20),vmean(0:3),cp,r,velm,tempm
!
      include "gauss.f"
      include "xlocal.f"
!
...
c
            ff(jj1)=ff(jj1)-xsjmod*dtl(konl(jj))*
     &            (cvel(1)*(shp(4,jj)+dtl(konl(jj))*shpv(jj)/2.d0)
c     &            (cvel(1)*(shp(4,jj)+dtimef*shpv(jj)/2.d0)
     &             +shp(1,jj)*t(1,1)+shp(2,jj)*t(1,2)+shp(3,jj)*t(1,3))
            ff(jj1+1)=ff(jj1+1)-xsjmod*dtl(konl(jj))*
     &            (cvel(2)*(shp(4,jj)+dtl(konl(jj))*shpv(jj)/2.d0)
c     &            (cvel(2)*(shp(4,jj)+dtimef*shpv(jj)/2.d0)
     &             +shp(1,jj)*t(2,1)+shp(2,jj)*t(2,2)+shp(3,jj)*t(2,3))
            ff(jj1+2)=ff(jj1+2)-xsjmod*dtl(konl(jj))*
     &            (cvel(3)*(shp(4,jj)+dtl(konl(jj))*shpv(jj)/2.d0)
c     &            (cvel(3)*(shp(4,jj)+dtimef*shpv(jj)/2.d0)
...
!     
     ...
     &                   corio(i1)*omcor
            ...
            jj1=1
            do jj=1,nope
               ff(jj1)=ff(jj1)+xsjmod*dtl(konl(jj))*bf(1)*(shp(4,jj)+
     &              dtl(konl(jj))*shpv(jj)/2.d0)
               ff(jj1+1)=ff(jj1+1)+xsjmod*dtl(konl(jj))*bf(2)*(shp(4,jj)
     &              +dtl(konl(jj))*shpv(jj)/2.d0)
               ff(jj1+2)=ff(jj1+2)+xsjmod*dtl(konl(jj))*bf(3)*(shp(4,jj)
     &              +dtl(konl(jj))*shpv(jj)/2.d0)
               jj1=jj1+3
            enddo
         endif
